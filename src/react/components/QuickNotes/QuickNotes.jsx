...
function QuickNotes() {
    ...
    const removeNotes = (itemId) => {
        ...
        localStorage.setItem('notes', JSON.stringify(List.filter((item) => item.key !== itemId)));
    }
        ...
        <ModalContext.Provider value={{ show: false }}>
            <Container className="rounded-4 bg-custom-color-grey-light ms-0 mt-5 me-5 pb-1 position-relative container-overflow position-relative" style={{maxHeight: "20px !important"}}>
                ...
                <ul className="p-0 m-0 !important">
                    {List.map((item) => <NoteItem  id={item.key} key={item.key} title={item.title} content={item.content} deleteNote={removeNotes}/>)}
                </ul>
            </Container>
            <ModalElement show={show} closeFunction={handleClose} saveChanges={addNotes} handleChange={handleInputChange} title="What would you like to take note of?" element1="Title" element2="Note"/>
